...
def get_questions():
    ...
    # total number of Qs from length
    ...
    # throw error if no questions
    if (len(current_questions) == 0):
        abort(404)

    # return questions, total num, and categories
    return jsonify({
        'success': True,
...
def delete_question(id):
    ...
    try:
        # find the question by ID, Either returns the question or NONE
        question = Question.query.filter_by(id=id).one_or_none()

        if question is None:
            # question by id not found throw error
            abort(404)
            # if found delete
        question.delete()
...
def post_question():
    ...
    if (body.get('searchTerm')):                # if search box filled
        ...
        # return all questions if a partial match on the search term
...
# if 1 or more form fields not filled out then throw the error below
        ...
        try:
            ...
            question.insert()
            # add new question to current questions return by ID
            selection = Question.query.order_by(Question.id).all()
            # paginate the questions including new one
            current_questions = paginate_questions(request, selection)
...
def get_questions_by_category(id):
    ...
    # check you have a record for that category
    category = Category.query.filter_by(id=id).one_or_none()
    if(category is None):
        abort(400)
    # load all questions where the category ID matches what was entered
...
def get_random_quiz_questions():
    ...
    # throw error is neither of the above
    ...
    if(category['id'] == 0):
        ...
        # or get all questions for that category
    ...
    def get_random_questions():
        ...
        # choose a random question between 0 and total number of questions
        return questions[random.randrange(0, len(questions), 1)]

    def used_questions(question):
        # has question been used. set to false
        used = False
        # check question against previous questions
        for q in previous:
